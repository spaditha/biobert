...
tf.flags.DEFINE_string("bioasq_version", None, "[Optional] TensorFlow master URL.")
tf.flags.DEFINE_string("bioasq_batch", None, "[Optional] TensorFlow master URL.")
...
def read_squad_examples(input_file, is_training):
  ...
  target_cnt = 500
  if is_training:
    pos_cnt = sum([1 for example in examples if example.answer == 1])
    neg_cnt = sum([1 for example in examples if example.answer == 0])
    print()
    ...
    for example in examples:
      if example.answer == 1 and new_pos_cnt >= target_cnt:
        continue
      if example.answer == 0 and new_neg_cnt >= target_cnt:
        continue
      else:
        new_examples.append(example)
        new_pos_cnt += (1 if example.answer == 1 else 0)
        new_neg_cnt += (1 if example.answer == 0 else 0)

    pos_cnt = sum([1 for example in new_examples if example.answer == 1])
    neg_cnt = sum([1 for example in new_examples if example.answer == 0])
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
  ...
  '''
  # Sigmoid weight
  output_weights_sgm = tf.get_variable(
      "cls/squad/output_weights_sgm", [1, hidden_size],
      initializer=tf.truncated_normal_initializer(stddev=0.02))
  output_bias_sgm = tf.get_variable(
      "cls/squad/output_bias_sgm", [1], initializer=tf.zeros_initializer())

  # Get first CLS token
  cls_hidden = final_hidden[:,0,:]
  # cls_hidden = tf.layers.dropout(cls_hidden, rate=0.5, training=is_training)

  # Standard logit
  '''
  logits = tf.matmul(cls_hidden, output_weights, transpose_b=True)
  logits = tf.nn.bias_add(logits, output_bias)
  '''

  # Sigmoid logit
  logits = tf.matmul(cls_hidden, output_weights_sgm, transpose_b=True)
  logits = tf.nn.bias_add(logits, output_bias_sgm)
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      def compute_loss(logits, targets):
        ...
        one_hot_positions = tf.one_hot(
            targets, depth=2, dtype=tf.float32)
      ...
      total_loss = compute_loss(logits, targets)
...
def write_predictions(all_examples, all_features, all_results, n_best_size,
  ...
  for (example_index, example) in enumerate(all_examples):
    ...
    for (feature_index, feature) in enumerate(features):
      ...
      # answer = 'yes' if np.argmax(logits) == 1 else 'no'
...
class FeatureWriter(object):
  ...
  def process_feature(self, feature):
    ...
    self.num_features += 1
    self.num_pos_features += 1 if feature.target == 1 else 0
...
def main(_):
  ...
  if FLAGS.do_predict:
    ...
    #output_prediction_file = os.path.join(FLAGS.output_dir, "predictions_{}b{}_{}{}.json".format(
    #    FLAGS.bioasq_version, FLAGS.bioasq_batch, FLAGS.bioasq_task, FLAGS.bioasq_snippet))
